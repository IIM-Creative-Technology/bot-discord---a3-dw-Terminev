...
module.exports.run = async (member) => {
  ...
  .then((resp) => {
    // Si la personne n'existe pas dans la BDD alors il est initialiasé 
    if(!resp[0]){
    ...
    const firstConnect = member.guild.roles.cache.map(r => r);
      ...
      for (var t = 0; t < firstConnect.length; t++) {
        // Si le rôle niveau 0 existe déjà sur le serveur alors il est ajouté
        if (firstConnect[t].name == "Niveau 0") {
          member.roles.add(firstConnect[t].id)
        } else {
          Role0 += 1
          // Si Role0 est égal au nombre de rôle présent sur le serveur, c'est que le role n'existe pas alors il est créé
