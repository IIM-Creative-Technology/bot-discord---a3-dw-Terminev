...
module.exports.run = async (client, message, arguments) => {
    db.executeQuery("SELECT * FROM xp WHERE user_id = " + message.author.id + " AND guild_id = " + message.guildId)
        .then((resp) => {
            // la const badWords permets de strocker tous les mots interdit
            ...
            for (var i = 0; i < badWords.length; i++) {
                ...
                if (message.content.split(' ').includes(badWords[i])) {
                    ...
                    message.delete();
                    // On récupere les avertissements de l'utilisateur et on incrémente le nombre d'avertissement
                    ...
                    // Si l'utilisateur atteint le nombre maximum d'avertissement alors il a role role Banned
                    if (Averto == 4) {
                        const role = message.guild.roles.cache.map(r => r)
                        var ban = false
                        for (var z = 0; z < role.length; z++) {
                            // Si le role Banned existe deja sur le serveur alors il est ajouté
